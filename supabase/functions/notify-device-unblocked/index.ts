...
serve(async (req) => {
  try {
    ...
    if (record.status !== 'pta_unblocked') {
      return new Response(JSON.stringify({ skipped: true }), { status: 200 })
    }

    const recipientId: string = record.requested_by
    const title = 'Device Unblocked'
    const body = 'Your device has been successfully unblocked by PTA.'
